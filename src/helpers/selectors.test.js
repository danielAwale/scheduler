...
test.only("getInterviewersByDay returns an array", () => {
  const result = getInterviewersByDay(state, "Monday");
  expect(Array.isArray(result)).toBe(true);
});

test.only("getInterviewersByDay returns an array with a length matching the number of appointments for that day", () => {
  const result = getInterviewersByDay(state, "Monday");
  expect(result.length).toEqual(3);
});

test.only("getInterviewersByDay returns an array containing the correct appointment objects", () => {
...
test.only("getInterviewersByDay returns an empty array when the days data is empty", () => {
  const result = getInterviewersByDay({ days: [] }, "Monday");
  expect(result.length).toEqual(0);
});

test.only("getInterviewersByDay returns an empty array when the day is not found", () => {
